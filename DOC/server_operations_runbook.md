...
| 组件/服务 | 监听端口 (Port) | 连接用户 (User) | 凭据来源 | 访问控制规则 |
| :--- | :--- | :--- | :--- | :--- |
| **MySQL** | `3306` | `<YASHE_DB_USERNAME>` | 权限为 `0600` 的 MySQL 配置文件 | 仅限本地 `127.0.0.1` 访问，禁止外网暴露 |
| **Redis** | `6379` | *无* | 服务器 Secret 文件 | 仅限本地 `127.0.0.1` 访问，禁止外网暴露 |
| **Spring Boot** | `8080` | `<NON_ROOT_DEPLOY_USER>` | Systemd 环境文件 | 内网由 Nginx 代理，不对公网开通 8080 安全组 |
| **Nginx (测试)** | `8000` | *无* | *无需密码* | 备案期间测试端口，限制为管理员 IP 访问 |
| **Nginx (正式)** | `80` / `443` | *无* | *无需密码* | 备案通过后正式启用 |
...
```bash
# 交互式输入密码，或使用权限为 0600 的 --defaults-extra-file
mysql -u '<YASHE_DB_USERNAME>' -p
```
在 MySQL 命令行内执行：
```sql
USE yashe_db;
